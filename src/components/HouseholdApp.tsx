...
export function HouseholdApp({
  ...
  const canManageProducts = membership.role !== 'assistant'
  const canArchiveProducts = membership.role === 'admin'
  ...
  async function adjustStock(productId: string, delta: number) {
    try {
      ...
      await repository.adjustStock(
        ...
        delta > 0 ? 'Actualización rápida' : 'Consumo rápido',
      )
      await refresh(true)
  ...
  async function archiveProduct(product: Product) {
    if (!window.confirm(`¿Archivar “${product.name}”? Dejará de aparecer en el inventario activo.`)) return
    try {
      await repository.archiveProduct(membership, product)
      await refresh(true)
      toast('Producto archivado', product.name)
    } catch (caught) {
      toast('No se pudo archivar', errorMessage(caught), 'error')
    }
  }
  ...
  async function createInvite(role: Exclude<HouseholdRole, 'admin'>, maxUses: number) {
    const invite = await repository.createInvite(membership, role, maxUses)
    await refresh(true)
    toast('Invitación creada', `Código ${invite.code}`)
  }

  async function revokeInvite(invite: HouseholdInvite) {
    await repository.revokeInvite(membership, invite)
    await refresh(true)
    toast('Invitación revocada', invite.code)
  }
  ...
  return (
    <>
      <div className="app-shell">
        <aside className={`sidebar ${mobileOpen ? 'open' : ''}`} aria-label="Navegación principal">
          ...
          <div className="sidebar-footer">
            ...
            <button className="secondary-button full" type="button" onClick={() => setQuickReviewOpen(true)}>Revisión rápida</button>
          </div>
        </aside>
        ...
        <div className="workspace">
          <header className="topbar">
            ...
            <div className="topbar-actions">
              ...
              {canManageProducts && <button className="primary-button" type="button" onClick={() => openProduct()}><span aria-hidden="true">＋</span>Agregar producto</button>}
          ...
          <main className="main-content">
            ...
            {view === 'shopping' && <ShoppingPage products={data.products} canEditShopping={canManageProducts} onPurchase={purchase} onSetShopping={setShopping} />}
            {view === 'history' && <HistoryPage movements={data.movements} />}
            {view === 'settings' && (
              <SettingsPage
      ...
      <nav className="mobile-bottom-nav" aria-label="Navegación móvil">
        <button className={`mobile-nav-item ${view === 'dashboard' ? 'active' : ''}`} type="button" onClick={() => navigate('dashboard')}><span>⌂</span><small>Inicio</small></button>
        <button className={`mobile-nav-item ${view === 'inventory' ? 'active' : ''}`} type="button" onClick={() => navigate('inventory')}><span>▦</span><small>Inventario</small></button>
        <button className="mobile-add-button" type="button" aria-label={canManageProducts ? 'Agregar producto' : 'Revisión rápida'} onClick={() => canManageProducts ? openProduct() : setQuickReviewOpen(true)}>＋</button>
        ...
        <button className={`mobile-nav-item ${view === 'settings' ? 'active' : ''}`} type="button" onClick={() => navigate('settings')}><span>⚙</span><small>Ajustes</small></button>
      </nav>
